 ...
 * bidirectional handling, template may be sent.
 *
...
static int ipfix_map[] = {
	/* XXX: current NFCT propagate delta count
...
static int nflow9_direction(struct ulogd_pluginstance *upi, u_int8_t *family,
...
static int stop_netflow9(struct ulogd_pluginstance *pi)
{
	...
	ulogd_log(ULOGD_ERROR, "iovcnt: %d\n", ii->iovcnt);
	if (ii->iovcnt > 1)
			      ...
			      struct ulogd_pluginstance_stack *stack)
{
	...
	for (i = 0; i < CII_MAX; i++)
		ii->ikey_count[i] = pi->input.num_keys;
	for (i = 0; i < pi->input.num_keys; i++) {
		for (j = 0; j < CII_MAX; j++) {
			if (!strncmp(pi->input.keys[i].name, count_keys[j],
				     strlen(count_keys[j]))) {
				ii->ikey_count[j] = i;
				break;
			}
